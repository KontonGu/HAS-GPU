...
def main():
    ...
    parser.add_argument("--url", required=True, help="URL for the client.py script (e.g., http://{gateway_host}:{gateway_port}/function/{model_name}/predict), port like 8080")
    parser.add_argument("--mode", required=True, help="Mode for the client.py script (e.g., bert)")
    parser.add_argument("--requests-file", required=True, help="Path to the requests file, eg. ../client/rps.json")
    parser.add_argument("--model-name", required=True, help="Model name for the client.py script (e.g., resnet)")
    ...
    run_command("cd ../../ && make fastgshare_deploy", "Failed to deploy HAS service")
    ...
    # run_command("cd /home/ubuntu/konton_ws/git_repo/github.com/KontonGu/Fast-Scaler && make has_sample", "Failed to deploy HAS sample")
    ...
    test_cmd = f"python ../client/test_batch.py --url {args.url}/function/{args.model_name}/predict --mode {args.mode} --duration 5"
    #test_cmd = f"python ../client/test_batch.py --url {args.url} --mode image --image ../client/car.jpg --duration 5"
    ...
    if args.mode == "bert":
        test_cmd = f"python ../client/test_batch.py --url {args.url}/function/{args.model_name}/predict --mode bert --request-counts-file ../trace/rps.json --request-data-output {args.output_dir}/fastgshare-{args.output_filename}.csv  > ./fastgshare-pylog.txt"
    else:
        test_cmd = f"python ../client/test_batch.py --url {args.url}/function/{args.model_name}/predict --mode image --image ../client/car.jpg --request-counts-file ../trace/rps.json --request-data-output {args.output_dir}/fastgshare-{args.output_filename}.csv  > ./fastgshare-pylog.txt"
    ...
    # 5. Copy CSV file
    usage_output_file = os.path.join(args.output_dir, f"fastgshare-usage-{args.output_filename}.csv")
    csv_copy_cmd = f"cp /data/gpu_usage/fastgshare_gpu_usage_data.csv {usage_output_file}"
    print(f"Step 5: Copying CSV file: {csv_copy_cmd}")
    run_command(csv_copy_cmd, "Failed to copy CSV file")
    
    print("Step 6: Removing Fast-Scaler...")
    #run_command("cd /home/ubuntu/konton_ws/git_repo/github.com/KontonGu/Fast-Scaler && make has_sample_remove", "Failed to remove HAS sample")
    run_command(f"kubectl delete -f {args.yaml}", "Failed to remove KServe InferenceService")

    # 7. Undeploy Fast-Scaler
    print("Step 7: Undeploying Fast-Scaler...")
    run_command("cd ../../ && make fastgshare_undeploy", "Failed to undeploy Fast-Scaler")
    ...
    # run_command("cp ../trace/pressure.json ./pressure.json", "Failed to copy request data")

    print("Step 8: Copying tmp py result...")
    run_command(f"cp ./fastgshare-pylog.txt {args.output_dir}/fastgshare-{args.output_filename}.log", "Failed to copy py log")
